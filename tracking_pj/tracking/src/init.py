...
class Frames():
    ...
    def get_init_tracker(self, tracker, tracker_id, tracker_cnt):
        """生成追踪器，需要考虑最后一帧无人的情况"""
        set_start_ID = [30 * i for i in range(self.cam_number)]
        for cam in range(self.cam_number):
            print('cam:', cam)
            tracker[cam] = self.rois[cam][-1]
            tracker_id[cam] = [i + set_start_ID[cam] for i in self.ids[cam][-1]]
            tracker_cnt[cam] = len(self.match_list_cam[cam]) + set_start_ID[cam]
    ...
    def get_all_IDs(self, ID_max):
        """获取当前库中的所有ID，生成当前可用ID列表"""
        ID_exist = []
        for cam in range(self.cam_number):
            for i in range(25):
                ID_exist.extend(self.ids[cam][i])
        ID_useful = []
        for i in range(ID_max):
            if i not in ID_exist:
                ID_useful.append(i)
        return ID_useful
...
def init_frames(cam_number, init_info):
    ...
    def roi_th_reid(cam, frame_id, init_info, th=0.3):  # 单帧数据
        frame_id, bbox, bbox_score, img = init_info[0][frame_id], init_info[1][frame_id][cam], init_info[2][frame_id][cam], init_info[3][frame_id][cam]
        if len(bbox) == 0:
            return [], [], [], []
        else:
    ...
    rois_init, features_init, imgs_init, dimGroup_init = \
        [[] for i in range(cam_number)], [[] for i in range(cam_number)], [[] for i in range(cam_number)], [[0] for i in range(cam_number)]  # 初始化
    ...
    print(len(imgs_init[1]))

    if len(features_init[0]) > 3 and len(features_init[1]) > 3:  # 若满足初始化条件    # and len(features_init[2]) > 3
        init_flag = 0
