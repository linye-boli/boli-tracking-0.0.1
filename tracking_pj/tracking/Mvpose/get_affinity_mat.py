...
def geometry_affinity(keyp, F, dimGroup):
    """
    ...
    Fs = [[F[1], F[0]],
          [F[1], F[0]]]
    pose_mat = []
    for cam in range(len(keyp)):
        for per in range(len(keyp[cam])):
            pose_mat.append(keyp[cam][per])
    pose_mat = np.array(pose_mat)

    M = pose_mat.shape[0]  # 共有M个人
    ...
    for i in range(len(dimGroup) - 1):
        for j in range(i, len(dimGroup) - 1):
            ...
            distance_matrix[dimGroup[i]:dimGroup[i + 1], dimGroup[j]:dimGroup[j + 1]] = \
                (projected_distance(pose_id0, pose_id1, Fs[i][j]) + projected_distance(pose_id1, pose_id0, Fs[j][i]).T) / 2
    ...
    geo_affinity_matrix = torch.tensor(geo_affinity_matrix)
