...
def get_match_list(reid_affinity_mat, geo_affinity_mat=[], dimGroup=None):
    # fuse two mats with multiplay
    if len(list(geo_affinity_mat)) == 0:
        W = reid_affinity_mat
    else:
        W = reid_affinity_mat
        # W = torch.Tensor(geo_affinity_mat)  # torch.sqrt(reid_affinity_mat * geo_affinity_mat)
