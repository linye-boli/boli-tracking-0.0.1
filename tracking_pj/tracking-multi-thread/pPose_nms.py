...
def pose_nms(ori_img,bboxes, bbox_scores, pose_preds, pose_scores):
    '''
    ...
    for j in range(len(pick)):
        ...
        bn = bboxes_pick[j].numpy()
        roi_temp = ori_img[int(bn[1]):int(bn[3]), int(bn[0]):int(bn[2])]
        roi_temp = Image.fromarray(roi_temp.astype('uint8')).convert('RGB')
        roi_temp = data_resize(roi_temp)
        roi.append(roi_temp)
        bbox.append(bboxes_pick[j])
        b_score.append(bbox_scores_pick[j])
        kp.append(keypoints)
        kp_score.append(merge_score)
    ...
    return [bbox],[b_score],[kp],[kp_score],[roi]
